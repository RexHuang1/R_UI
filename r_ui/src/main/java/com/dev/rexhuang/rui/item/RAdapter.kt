 ...
 **  on 2020/7/27
 */
class RAdapter(context: Context) : Adapter<ViewHolder>() {
    private var recyclerViewRef: WeakReference<RecyclerView>? = null
    private var mContext: Context = context
    private var mInflater = LayoutInflater.from(context)
    private var dataSets = ArrayList<RDataItem<*, out ViewHolder>>()
    private var typeArrays = SparseArray<RDataItem<*, out ViewHolder>>()

    private var headers = SparseArray<View>()
    private var footers = SparseArray<View>()

    private var BASE_ITEM_TYPE_HEADER = 1_000_000
    private var BASE_ITEM_TYPE_FOOTER = 2_000_000
     ...
     * 往现有集合的尾部添加items集合
     */
    fun addItems(items: List<RDataItem<*, out ViewHolder>>, notify: Boolean) {
     ...
     * 移除指定item
     */
    fun removeItem(dataItem: RDataItem<*, out ViewHolder>) {
     ...
     * 指定刷新 某个item的数据
     */
    fun refreshItem(dataItem: RDataItem<*, out ViewHolder>) {
    ...
    private fun createViewHolderInternal(
        javaClass: Class<RDataItem<*, out ViewHolder>>,
        view: View
    ): ViewHolder {
        //得到该Item的父类类型,即为RDataItem.class。  class 也是type的一个子类。
        ...
        return object : RViewHolder(view) {}
    }
    ...
    fun getItem(position: Int): RDataItem<*, ViewHolder>? {
        if (position < 0 || position >= dataSets.size)
            return null
        return dataSets[position] as RDataItem<*, ViewHolder>
    }
